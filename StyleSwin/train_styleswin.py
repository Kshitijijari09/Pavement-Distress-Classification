    ...
    for idx in range(args.iter):
        ...
        if get_rank() == 0:
            if i % args.print_freq == 0:
                ...
                if args.tf_log:
                    vis.plot_dict(vis_loss, step=(i * args.batch * int(os.environ["WORLD_SIZE"]), args.iter))
...
def evaluation(generator, args, steps, iternum):
    cnt = 0
    ...
    for _ in range(args.val_num_batches):
        with torch.no_grad():
            ...
            if not os.path.exists(os.path.join(args.sample_path, "eval_{}".format(str(steps)))):
                os.mkdir(os.path.join(args.sample_path, "eval_{}".format(str(iternum))))

            for j in range(args.val_batch_size):
                torchvision.utils.save_image(
                    out_sample[j],
                    os.path.join(args.sample_path, "eval_{}".format(str(iternum))) + f"/{str(cnt).zfill(6)}.png",
                    nrow=1,
                    padding=0,
                    normalize=True,
                    range=(0, 1),
...
if __name__ == "__main__":
    ...
    #jjjj
    parser.add_argument('--val_num_batches', default=1, type=int, help='Num of batches will be generated during evalution')
    parser.add_argument('--val_batch_size', default=200, type=int, help='Batch size during evalution')
    ...
    #decay 
    parser.add_argument("--lr_decay_start_steps", default=300000, type=int, help='Steps to start lr decay')
